$(document).ready(function() {
    ...
    $.getJSON({
        url: "https://raw.githubusercontent.com/carpyncho/carpyncho-py/master/data/index.json?v=202206",
        cache: false
    }).then(json => {
        ...
        $.each(tdata, function(lname, ldata) {
            ...
            $otherUl.find(`#${ocid} > a`).click(function(evt) {
                ...
                var links = " ".concat(
                    ...
                    `<a href="${url}" class="">`,
                    `<i class="fas fa-download"></i> ${ldata["hname"]}`,
                    "</a>",
        ...
        $clickeable.click(function(evt) {
            ...
            $.each(tdata, function(lname, ldata) {
                ...
                var links = " ".concat(
                    ...
                    "<div class='d-flex justify-content-between align-items-center'>",
                    `<a href="${url}" class="">`,
                    `<i class="fas fa-download"></i></i> ${ldata["hname"]}`,
                    "</a>",
                    `<span class="badge badge-info badge-pill">${size}</span>`,
                    `<span class="badge badge-secondary badge-pill">Updated ${ldata['date']}</span>`,
                    "</div>",
